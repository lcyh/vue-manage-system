<template>
    <div class="label-wrapper label2">
        <div class="head">
            <div>
        ...
        <div class="card-content">
            <el-form ref="form" :rules="rules" :model="formModel" label-width="80px">
                ...
                <el-form-item label="标签规则" prop="labelRule.value">
                    <span class="label-name">首次登录日期</span>
                    <el-date-picker @change="selectLabelChange" v-model="formModel.labelRule.value" type="date" placeholder="选择日期">
                    </el-date-picker>
                </el-form-item>
            </el-form>
        </div>
...
export default {
    ...
    computed: {
        ...
        formModel() {
            let value = this.item.game || this.item.game;
    ...
    methods: {
        handleDeleteLabel() {
            let value = {
                ...
                labelRule: {
                    value: this.formModel.labelRule.value ? moment(this.formModel.labelRule.value).format('YYYY-MM-DD') : undefined,
                    unit: undefined
                }
            };
